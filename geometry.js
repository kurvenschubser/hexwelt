...
// a² + b² = c² <=> b² = c² - a² => b² = 1 - 0.25 = 0.75 => b = sqrt(0.75)
...
Hexagon.prototype.containsPoint = function (point) {
  ...
  if (
    new Rectangle(this.x, this.y + this.r / 2, width * 2, this.r).containsPoint(
      point
    )
  ) {
    return true;
  ...
  var ydistance = Math.abs(
    this.y + (point.y > center.y) * this.r * 2 - point.y
  );
  if (ydistance > this.r / 2 - gradient * xdistance) return true;
  return false;
};
